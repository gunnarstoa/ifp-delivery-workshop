...
class PageOneBG(Flowable):
    """Full bleed cover background for page 1."""
    def wrap(self, avW, avH):
        return 0, 0
...
class PageTwoBG(Flowable):
    """Background for page 2 (clean white with accent elements)."""
    def wrap(self, avW, avH):
        return 0, 0
...
def build_doc():
    ...
    p2_tmpl    = PageTemplate(id='Content', frames=[p2_frame])
    ...
    # ── PAGE 1 (cover — rendered by PageOneBG; story is empty placeholder) ──
    story.append(PageOneBG())
    story.append(NextPageTemplate('Content'))
    story.append(PageBreak())

    # ── PAGE 2 ──────────────────────────────────────────────────────────────────
    story.append(PageTwoBG())
